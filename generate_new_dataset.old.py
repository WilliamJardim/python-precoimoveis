...
def generate_realistic_bought(price, location_score, has_garden, has_garage, income_level):
    # Higher income increases likelihood of buying higher-priced homes
    affordability_score = income_level / max(1, price / 100000)
    desirability = location_score + (has_garden * 2) + (has_garage * 1.5)
    # Combine factors into a probability
    probability = (affordability_score + desirability) / 20
    return 1 if random.uniform(0, 1) < probability else 0
"""

"""
def generate_realistic_bought(price, location_score, has_garden, has_garage, income_level):
    # Ajuste para probabilidade com base no preço do imóvel
    probability = max(0.05, min(1.0, (income_level / 100000) - (price / 500000)))

    # Fatores adicionais para influência
    probability += (location_score / 10) + (has_garden * 0.2) + (has_garage * 0.1)

    # Gera um valor aleatório para determinar se um imóvel foi comprado
    return 1 if random.uniform(0, 1) < probability else 0    
"""
...
realistic_output_path = "./realistic_real_estate_dataset-novo.json"
